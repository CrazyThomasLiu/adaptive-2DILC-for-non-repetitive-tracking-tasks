...
for time in range(batch_length):
    omega_t[time,0]=omega_t[time,0]*10*np.sin(0.2*time)
    omega_t[time, 1] = omega_t[time, 1] *10* np.cos(0.2*time)
...
for time in range(batch_length):
    #1
    #A_hat_t[time]=A*(0.3+0.1*np.exp(time/200))
    #B_hat_t[time]= B*(0.8+0.05*np.exp(time/50))
    #C_hat_t[time] = C+ 0.5*delta_C_t * np.sin(time)

    #2
    #A_hat_t[time]=A*(0.65+0.05*np.exp(time/200))
    #B_hat_t[time] = B * (0.8 + 0.02 * np.exp(time / 50))
    #C_hat_t[time] = C+ 0.5*delta_C_t * np.sin(time)
    A_hat_t[time]=A*0.65
    B_hat_t[time] = B * 0.8
    C_hat_t[time] = C+ delta_C_t

    #A_hat_t[time]=A*(0.35+0.1*np.exp(time/200))
    #B_hat_t[time]= B*(0.7+0.05*np.exp(time/50))
    #C_hat_t[time] = C+ 0.5*delta_C_t * np.sin(time)
...
for time in range(batch_length):
    #A_t.append(copy.deepcopy(A))
    A_hat_t[time]=A*(0.2+0.1*np.exp(time/200))
    #A_t[time][0, 0] = A_t[time][0, 0] * np.exp(time / 100)
    #B_t.append(copy.deepcopy(B))
    B_hat_t[time]= B*(0.5+0.05*np.exp(time/50))
    #B_t[time][0, 0] = B_t[time][0, 0] * np.exp(time / 100)
    #C_t.append(copy.deepcopy(C))
    C_hat_t[time] = C+ 0.5*delta_C_t * np.sin(time)
'1.3 add a additional parameter for the t+1'
C_hat_t[batch_length] = C+ 0.5*delta_C_t * np.sin(batch_length)
"""
...
norm_C_t=[]
norm_sum_A_B=[]
sum_A_B_t=np.concatenate((A_t, B_t), axis=2)
#pdb.set_trace()
...
sum_A_B_hat=np.concatenate((A_hat_t, B_hat_t), axis=2)
norm_A_t.append(np.linalg.norm(A_t - A_hat_t))
norm_B_t.append(np.linalg.norm(B_t - B_hat_t))
norm_C_t.append(np.linalg.norm(C_t - C_hat_t))
#norm_sum_A_B.append(np.linalg.norm(sum_A_B_t - sum_A_B_hat))
norm_sum_A_B.append(np.linalg.norm(sum_A_B_t[20] - sum_A_B_hat[20]))
for batch in range(batch_num):

    #sys_model = ILI(batch_length=batch_length, A_t=A_t, B_t=B_t, C_t=C_t, Q_t=Q_t, R_t=R_t)
    #mbilc = MBNTILC(batch_length=batch_length, A_t=A_hat_t, B_t=B_hat_t, C_t=C_hat_t, Q_t=Q_t, R_t=R_t)
    #mbilc.computation()
    ...
    #x_k0_random = x_k0 * (np.ones((1,3))+np.random.uniform(-1,1,(1,3))).reshape((3))
    x_k0_random = x_k0
    #x_k0_random = np.array((0., 0., 0.))
    #print(x_k0_random)
    #pdb.set_trace()
    #state, output_info,error_info,ILI_info = controlled_system.reset(y_ref=y_ref)
    ...
    # TODO: initial state are all zero 1. 第一个批次的数据。2.更改初始状态不为0
    ILI_delta[0]=ILI_info[0]
    ILI_x[0] = ILI_info[1]
    ILI_y[0] = ILI_info[2]
    #ILI_y[0]=copy.deepcopy(output_info)
    #pdb.set_trace()
    for time in range(batch_length):
        ...
        #ILI_y[time+1] = copy.deepcopy(output_info)
    #pdb.set_trace()
    #controlled_system.save_initial_data()
    #pdb.set_trace()
    ...
    #pdb.set_trace()
#for inx in range(batch_num+1):
#    norm_sum_A_B.append(norm_A_t[inx]+norm_B_t[inx])
...
if save_figure==True:
    plt.savefig('mfilc_response.pdf')
...
#ax1.set_title('10-iteration $x_{1}$')
if save_figure==True:
    plt.savefig('mfilc_control_signal.pdf')
    #plt.savefig('mfoilc_response_PILC.jpg',dpi=700)
...
plt.plot(batch_time,norm_sum_A_B[1:],linewidth=1.5,color='tab:blue',linestyle = 'dashdot')
#plt.plot(batch_time,y_2dilc_rl_show,linewidth=1.5,color='tab:orange',linestyle='solid')
plt.grid()

xlable = 'Batch:$\mathit{k} $'
ylable = 'energy function'
